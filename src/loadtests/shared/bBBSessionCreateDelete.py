...
def bBBSharedTest(session):
    ...
    driverWB = webdriver.Remote(f"http://{Constant.browserIpPort}/wd/hub", DesiredCapabilities.CHROME) # browser which will be used for creating the BBB rooms
    ...
    while counterfirst < int(Constant.numberRooms):
        ...
        operator = "create"
        urlparam = f"meetingID=loadtest-{timestamp}{str(counterfirst)}&name=loadtest-{str(time.time())}{str(counterfirst)}&moderatorPW=123&attendeePW=456&lockSettingsDisableMic=true"
        ...
        operator = "join"
        urlparam = f"meetingID=loadtest-{timestamp}{str(counterfirst)}&fullName=loadtest-{str(counterfirst)}userMLoadtest-{str(countersecond)}&userID=loadtest-{str(counterfirst)}userMLoadtest-{str(countersecond)}&password=123"
        urlstart = f"{Constant.bBBHost}/bigbluebutton/api/{operator}?{urlparam}"
        url = str(operator) + str(urlparam) + str(Constant.bBBKey)
        ...
        while countersecond < int(Constant.numberUsers):
            ...
            operator = "join"
            urlparam = f"meetingID=loadtest-{timestamp}{str(counterfirst)}&fullName=loadtest-{str(counterfirst)}userLoadtest-{str(countersecond)}&userID=loadtest-{str(counterfirst)}userLoadtest-{str(countersecond)}&password=456"
            urlstart = f"{Constant.bBBHost}/bigbluebutton/api/{operator}?{urlparam}"
            url = str(operator) + str(urlparam) + str(Constant.bBBKey)
    ...
    while counterfirst < int(Constant.numberRooms):
        ...
        operator = "end"
        urlparam = f"meetingID=loadtest-{timestamp}{str(counterfirst)}&password=123"
        urlstart = f"{Constant.bBBHost}/bigbluebutton/api/{operator}?{urlparam}"
        url = str(operator) + str(urlparam) + str(Constant.bBBKey)
        urlsha = f"{str(urlstart)}&checksum={hashlib.sha1(url.encode()).hexdigest()}"
